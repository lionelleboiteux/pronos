-- Jeu des Pronos — Supabase Postgres schema
-- Base design by Lio (see ../context/supabase_score_prediction_schema.sql).
-- Changes made at the architecture gate, each traced to a spec requirement:
--   1. players.email made nullable (was `not null unique`) — AC-12 requires
--      email to be optional; submission must succeed with no email and send
--      nothing. Postgres UNIQUE already permits any number of NULLs, so no
--      other change is needed to keep email unique when present.
--   2. leagues.config jsonb added — AC-13 requires a 4th league to be added by
--      configuration alone (fixture source mapping, gameweek calendar), not by
--      changing the data model or form logic.
--   3. leagues.current_gameweek_id added as a read-side cache only. It speeds
--      up "what should the form show" queries; it is never the authority for
--      whether a specific match accepts a submission — that check always goes
--      straight to games.starts_at at request time (see AC-01/AC-02 notes on
--      the prediction-submit endpoint in contracts/openapi.yaml). A stale
--      cache pointer can make the UI slow to update; it cannot make the
--      backend accept a late pick.
--   4. duplicate_flags table added — AC-09 requires near-duplicate pseudos to
--      be surfaced together in a weekly admin review queue, without blocking
--      or auto-merging either submission.
--   5. telemetry_events table added — the 8 events in the spec header are
--      requirements as binding as any acceptance criterion (see
--      pdlc/jeu-des-pronos/spec-header.json). This is where John's dashboard
--      reads from.
