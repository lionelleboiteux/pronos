...
- **Scoring can run before a gameweek's matches are actually finished:** `runScoring()` gates on kickoff time (`isLocked`), not on `games.status = 'finished'`, so the pg_cron tick can score a gameweek's placeholder/interim data within a minute of the last kickoff — hours before final results are ingested — and the `already_completed` guard then prevents any automatic correction. See "Classement Général Per-Gameweek Breakdown" above for a confirmed occurrence and the manual rescore workaround; the trigger-timing fix itself is not yet made.
